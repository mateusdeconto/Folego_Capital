...
export default function Onboarding({ onComplete, onBack, initialData = null, user = null, existingDiagnoses = [] }) {
  ...
  const selectedSeg = SEGMENTS.find(item => item.value === segment);
  ...
  const companyLabel = customSegment?.trim() || selectedSeg?.label || segment;
      ...
      <div className="mb-7">
        ...
        <h1 className="text-[2rem] sm:text-[2.4rem] font-bold text-ink-900 tracking-tighter leading-[1.1] mb-3">
          {isReusingCompany ? (
            <>
              Nova DRE para<br />
              <span className="text-ink-400">{businessName}.</span>
            </>
          ) : greeting ? (
            <>
              {greeting}<br />
        ...
        <p className="text-ink-500 text-base leading-relaxed">
          {isReusingCompany
            ? 'Empresa e setor já definidos. Agora falta só escolher mês da nova análise.'
            : 'Cada setor tem realidades diferentes e, sabendo o seu, comparamos com PMEs parecidas com a sua.'}
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-7">
        {isReusingCompany ? (
          <div className="rounded-2xl bg-white border border-ink-200 p-4">
            ...
            <p className="text-lg font-bold text-ink-900">{businessName}</p>
            <p className="text-sm text-ink-500 mt-0.5 capitalize">{companyLabel}</p>
          </div>
        ) : (
          <div>
        ...
        <button type="submit" disabled={!canProceed} className="btn-primary">
          <span className="flex items-center justify-center gap-2">
            {canProceed ? (isReusingCompany ? 'Continuar com essa empresa' : 'Bora começar') : 'Preencha pra continuar'}
